...
We would love to accept your patches and contributions to the Next Mavens CLI project.
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/next-mavens.js` within the `packages/cli` directory, pausing execution until a debugger attaches. You can then open `chrome://inspect` in your Chrome browser to connect to the debugger.
