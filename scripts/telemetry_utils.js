...
// User-level .nextmavens directory in home
const USER_NEXTMAVENS_DIR = path.join(os.homedir(), '.nextmavens');
// Project-level .nextmavens directory in the workspace
const WORKSPACE_NEXTMAVENS_DIR = path.join(projectRoot, '.nextmavens');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.nextmavens/tmp
export const OTEL_DIR = path.join(USER_NEXTMAVENS_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .nextmavens directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_NEXTMAVENS_DIR,
  'settings.json',
);
